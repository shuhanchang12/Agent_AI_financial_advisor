...
const API_BASE_URL = 'http://localhost:8000'; // As specified in the upgrade report
...
const mockApi = {
    async runAnalysis(ticker: string, query: string, file?: File): Promise<AnalysisResponse> {
        console.log(`[Frontend->Backend] Starting analysis for ${ticker} with query: "${query}"`);
        if (file) {
            console.log(`[Frontend->Backend] Including uploaded file: ${file.name}`);
        }
    ...
    async getAnalysisState(threadId: string): Promise<StateResponse> {
        ...
        if (!window.mockState[threadId]) {
            ...
            const dataStatus = initialData.dataSource.includes('(Live)') ? 'Verified' : 'Stale';
             ...
             window.mockState[threadId] = {
                ...
                initialData: initialData // Keep initial data for reference
            };
        }
        ...
        const fileName = state.initialData.fileName;
        ...
        const vectorRetrievalContent = fileName
            ? `Querying uploaded document "${fileName}" and internal vector database for "${state.report.ticker}". Retrieved 5 relevant chunks from your document and 2 from our knowledge base.`
        ...
        const vectorRetrievalCitations = fileName
            ? [{uri: '#', title: `Uploaded: ${fileName}`}, {uri: '#', title: 'DocID: Q2_2024_Earnings_Transcript'}, {uri: '#', title: 'DocID: DCF_Model_July_2024'}]
        ...
        const agents: { role: ChatRole, content: string, citations?: any[] }[] = [
            { role: 'Planner', content: `Initiating comprehensive analysis for ${state.report.ticker}. Plan: 1) Verify real-time price. 2) Query internal knowledge base for historical performance. 3) Execute web search for Q3 earnings reports and recent analyst ratings. 4) Delegate findings to Technical, Macro, and Sentiment analysts for specialized review.` },
    ...
    async chatWithCoPilot(threadId: string, message: string): Promise<{ report: AnalysisReport }> {
        ...
        if (lowerMessage.includes('aggressive') || lowerMessage.includes('more risk')) {
            ...
            coPilotResponse = "That's a great question. Based on the analysis, the primary risk is macro-economic uncertainty. The technicals suggest upside, but a hawkish stance from the FOMC could invalidate the bullish thesis. Diversifying with a small hedge might be a prudent approach.";
 ...
 * @param file Optional file for RAG context.
 * @returns A promise that resolves with the initial response from the backend, including a thread_id.
 */
export const runAnalysis = async (ticker: string, query: string, file?: File): Promise<AnalysisResponse> => {
...
/**
 * Sends a chat message to the Co-Pilot agent for interactive strategy refinement.
 * @param threadId The ID of the workflow thread.
 * @param message The user's message to the agent.
 * @returns A promise that resolves with the fully updated report after the agent's response.
 */
export const chatWithCoPilot = async (threadId: string, message: string): Promise<{ report: AnalysisReport }> => {
    return mockApi.chatWithCoPilot(threadId, message);
}
